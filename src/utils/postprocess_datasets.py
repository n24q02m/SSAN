...
def filter_images(dataset_path, min_bbox_ratio=0.05):
    ...
    for folder in ["live", "spoof"]:
        ...
        with mp.Pool(num_processes) as pool:
            results = list(
                tqdm(
                    pool.imap(
                        partial(check_and_remove_file, min_bbox_ratio=min_bbox_ratio),
                        image_files,
                    ),
    ...
    for folder in ["live", "spoof"]:
        removed = sum(
            stats[folder][key] for key in ["no_bbox", "invalid_image", "small_bbox"]
        )
...
def main():
    for name, path in DATASETS.items():
        ...
        if create_zips(path):
            ...
            print(
                "Please upload live.zip and spoof.zip manually through the Kaggle website"
            )
        else:
