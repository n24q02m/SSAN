...
def save_sample_images(dataset_path, num_samples=5):
    ...
    for folder in ['live', 'spoof', 'live/live', 'spoof/spoof']:
        ...
        selected_with_bbox = random.sample(with_bbox, min(num_samples, len(with_bbox))) if with_bbox else []
        selected_without_bbox = random.sample(without_bbox, min(num_samples, len(without_bbox))) if without_bbox else []
        ...
        for i, img_path in enumerate(selected_with_bbox):
            try:
                ...
                cv2.putText(img_with_bbox, label, (int(x), int(y-10)), 
