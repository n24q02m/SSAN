...
class TestPredictor:
    ...
    def test_init(self, setup):
        ...
        assert str(self.predictor.output_dir).startswith(
            str(self.output_dir)
        )  # Chỉ check prefix
        # Check directory structure
        for dir_path in [
            self.predictor.csv_dir,
            self.predictor.plot_dir,
            self.predictor.log_dir,
        ]:
    ...
    def test_predict_batch(self, setup):
        ...
        if self.predictor.device != "cpu":
            assert next(self.predictor.model.parameters()).device == torch.device(
    ...
    def test_save_results(self, setup):
        ...
        metrics = {"accuracy": 0.75, "auc": 0.8, "tpr@fpr=0.01": 0.7, "hter": 0.2}
