...
class Predictor:
    ...
    def predict_batch(self, batch: tuple) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            pred = (
                F.adaptive_avg_pool2d(pred, 1).squeeze(-1).squeeze(-1).squeeze(-1)
            )  # [B]
    ...
    def save_results(
        self, results: Dict[str, np.ndarray], metrics: Dict[str, float]
    ) -> None:
    ...
    @classmethod
    def from_checkpoint(
        cls, checkpoint_path, model, test_loader, device, output_dir=None
    ):
        ...
        return cls(
            model=model, test_loader=test_loader, device=device, output_dir=output_dir
        )
