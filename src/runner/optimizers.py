...
def find_optimal_batch_size(
    ...
    tried_sizes = []
    with tqdm(
        total=int(np.log2(max_batch - min_batch + 1)) + 1, desc="Testing batch sizes"
    ) as pbar:
        while left <= right:
            mid = (left + right) // 2
...
def find_optimal_workers(dataloader: DataLoader, max_workers: int = None) -> int:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        if params["scheduler"] == "step":
            ...
            scheduler = lr_scheduler.CosineAnnealingWarmRestarts(
                optimizer, T_0=params["warmup_epochs"], eta_min=params["min_lr"]
            )
        ...
        train_subset_size = int(
            len(self.train_loader.dataset) * self.optimization_fraction
        )
        ...
        train_subset = Subset(
            ...
            indices=self._get_balanced_indices(
                self.train_loader.dataset, self.optimization_fraction
            ),
        )
        val_subset = Subset(
            self.val_loader.dataset,
            indices=self._get_balanced_indices(
                self.val_loader.dataset, self.optimization_fraction
            ),
        )
        ...
        for epoch in range(n_epochs):
            ...
            pd.DataFrame(
                ...
                metrics_file, mode="a", header=not metrics_file.exists(), index=False
            )
            ...
            if consecutive_poor >= max_poor:
                self.logger.info(
                    f"Trial pruned due to poor performance: accuracy={accuracy:.6f}"
                )
