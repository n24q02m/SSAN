...
class SSAN(nn.Module):
    ...
    def __init__(self, num_domains=5, ada_blocks=2, max_iter=4000, dropout=0.0):
        ...
        self.ada_blocks = nn.ModuleList([AdaIN(eps=1e-5) for _ in range(ada_blocks)])

        # Final conv after AdaIN blocks
        self.conv_final = nn.Sequential(
            nn.Conv2d(256, 512, 3, stride=2, padding=1, bias=False), nn.BatchNorm2d(512)
        )
    ...
    def shuffle_style_assembly(
        self, x, live_spoof_labels, domain_labels, lambda_val=1.0
    ):
        ...
        # Forward passes
        pred_orig, domain_pred, feats_orig = self.forward(
            x, domain_labels, return_feats=True, lambda_val=lambda_val
        )
        _, _, feats_shuffled = self.forward(
            x_shuffled,
            domain_labels[rand_idx],
            return_feats=True,
        ...
        return (
            pred_orig,
            domain_pred,
            feats_orig["style_feat"],
            feats_shuffled["style_feat"],
            contrast_labels,
        )
